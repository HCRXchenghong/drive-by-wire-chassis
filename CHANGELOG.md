...
- CAN 启动失败后每 `500 ms` 自动重试。
- 重试前先调用 `HAL_CAN_Stop()`，再重新配置滤波器和启动通知，避免 CAN 处于半启动状态后一直卡住。
- 保留现场确认过的转向策略：远处固定 `1000 RPM` 上限，越接近目标越慢，摇杆边界按 95% 机械限位映射，校准点动松手后不自动回中。

### 修复

- 修复 App 已连接、`ble_connected=1 / rs=APP_JSON / rfc=2000`，但 `dc=0 / sc=0` 后无法恢复的问题。
- 机械转向最大输出仍为固件固定值 `1000 RPM`；`steering_max_rpm` 的 App/Flash 可配置链路属于后续 `v2.0.1`。
