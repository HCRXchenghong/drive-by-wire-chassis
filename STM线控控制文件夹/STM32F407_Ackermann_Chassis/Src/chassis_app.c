...
/* MSSC 转向电机闭环输出上限。
 * 取 300 RPM，对常见 5:1 减速比对应输出端约 60 RPM (≈360°/s)，
 * 锁到锁打满约 0.3s，不至于撞限位太猛，又能快速跟上摇杆。 */
#define CONTROL_DEFAULT_STEER_MAX_RPM 2000U
#define CONTROL_MIN_STEER_MAX_RPM 200U
#define CONTROL_MAX_STEER_MAX_RPM 5000U
...
static float control_steering_max_rpm(const vehicle_config_t *config);
                                                 ...
                                                 float *actual_angle_deg)
{
  ...
  /* 比例项：目标 RPM = KP * 误差。
   * 大误差时由配置里的 steering_max_rpm 钳位，小误差时由 MSSC 最低门
   * 限兜底，方向永远跟随 angle_error 的符号——不依赖 KP*err 自身的
   * 符号，因此 KP=0 或 err≈0 时仍然方向明确。 */
  rpm_command = abs_error * STEERING_POSITION_KP;
  rpm_command = clamp_float(rpm_command, 0.0f, control_steering_max_rpm(config));
...
static float control_steering_max_rpm(const vehicle_config_t *config)
{
  uint16_t steering_max_rpm = CONTROL_DEFAULT_STEER_MAX_RPM;

  if ((config != NULL) &&
      (config->steering_max_rpm >= CONTROL_MIN_STEER_MAX_RPM) &&
      (config->steering_max_rpm <= CONTROL_MAX_STEER_MAX_RPM))
  {
    steering_max_rpm = config->steering_max_rpm;
  }

  return (float)steering_max_rpm;
}
...
static void publish_remote_status_if_needed(void)
{
  ...
  (void)snprintf(
      ...
      "\"dmr\":%u,\"smr\":%u,\"ls\":%u,\"sid\":%u,\"hid\":%u,\"ldi\":%u,\"rdi\":%u,\"lsd\":%u,"
      "\"thr\":%u,\"brk\":%u,"
      ...
      (unsigned int)config->steering_max_rpm,
...
static void reply_config(void)
{
  ...
  (void)reply_printf(
      ...
      "\"drive_max_rpm\":%u,\"steering_max_rpm\":%u,"
...
static void handle_set_config(const char *line)
{
  ...
  if (json_get_uint32(line, "\"steering_max_rpm\"", &value))
  {
    config.steering_max_rpm = (uint16_t)value;
  }
